...
class ResultCollector:
    ...
    def collect_scenario_results(self, scenario_name: str, test_results: Dict[str, TestResult],
        ...
        self.logger.info(f"Collecting results for scenario {scenario_name} from {len(node_names)} nodes")
        ...
        remote_paths = self._get_remote_result_paths(node_names)
        ...
        try:
            ...
            # 收集文件
            collected_files = self._collect_files_from_nodes(task)
            task.collected_files = collected_files
            
            # 保存测试结果元数据
            self._save_test_metadata(scenario_result_dir, test_results)
            
            # 生成结果摘要
            summary = self._generate_result_summary(scenario_name, timestamp, test_results, collected_files)
            
            # 保存摘要
            self._save_result_summary(scenario_result_dir, summary)
            
            # 生成报告
            self._generate_reports(scenario_result_dir, summary, test_results)
    ...
    def _get_remote_result_paths(self, node_names: List[str]) -> List[str]:
        """获取远程结果路径"""
        paths = []
        for node_name in node_names:
            node = self.node_manager.get_node(node_name)
            if node:
                paths.append(node.results_path)
        return list(set(paths))  # 去重
    
    def _collect_files_from_nodes(self, task: CollectionTask) -> List[str]:
        """从节点收集文件"""
        collected_files = []
        ...
        for node_name in task.node_names:
            ...
            try:
                # 收集结果文件
                node_files = self._collect_files_from_single_node(
                    node, node.results_path, str(node_result_dir)
                )
    ...
    def _generate_result_summary(self, scenario_name: str, timestamp: str,
    ...
    def _generate_reports(self, result_dir: Path, summary: ResultSummary,
                         test_results: Dict[str, TestResult]):
        """生成报告"""
        try:
            # 生成CSV格式的性能报告
            self._generate_performance_csv(result_dir, test_results)
    ...
    def _generate_performance_csv(self, result_dir: Path, test_results: Dict[str, TestResult]):
        """生成性能数据CSV"""
        try:
            data = []
            for node_name, result in test_results.items():
